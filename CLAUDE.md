...
- **The grid is grouped into collapsible sections too**, mirroring the channel list: `★ FAVORITES` then one per group, alphabetical. `build` returns `list[Section]`; `lines()` flattens them into drawing order, header first, rows only when expanded. Keeping that flattening in `grid.py` is what makes the index-to-content mapping — which is all hit testing needs — testable without a display, since the grid is a Canvas with no widget hierarchy to interrogate.
- **A favorite appears *once*, under FAVORITES, not again in its group.** The channel list can afford that duplication because its rows are one line each; here it would draw the same schedule twice, which is exactly what deduplicating by tvg-id exists to prevent. Favoriting one side of a shared tvg-id also decides which name wins the row.
- **The guide keeps its own collapsed set** in `Settings.collapsed_guide_groups`, deliberately not shared with the list's `collapsed_groups`: folding the list down to launch quickly should not empty the window you open to see what is on.
- **Section headers span both canvases and both accept the click.** The band across the grid side is not decoration — it is the click target there, and `_on_click` checks for a header *before* looking for a programme, so a heading can never play a channel. `_toggle_section` is bound directly on the names canvas.
- **The filter box reuses `search.filter_channels`**, so matching is identical to the main list, accents and all. Three things it has to get right: the scroll resets to the top on every query change (filtering 121 rows to three while scrolled near the bottom otherwise shows blank canvas); the footer counts against the *rows there could be* rather than the whole playlist, since "3 of 482" would imply the filter discarded channels that never had guide data; and `Escape` clears the filter when there is one and only closes the window when there is not, matching the main window's `Esc`.
- **The arrow-key guard belongs on the binding, not on `_page_back`/`_page_on`.** `<Left>`/`<Right>` are bound on the toplevel so they fire whatever holds focus, and Tk's Entry class binding has already moved the caret by then — paging as well would make the arrows useless for editing. But the toolbar buttons call the same two methods, so guarding *there* would make clicking ◀ silently do nothing whenever the filter had focus. `_arrow` wraps them for the key path only, and the buttons take `takefocus=False` so paging never steals focus mid-typing.
- **`grid.Names` is a Protocol, not `Container[str]`.** `Container` requires `__contains__(object)` and `storage.Favorites` narrows its parameter to `str`, so it does not satisfy it; a set of names satisfies the Protocol too. Same reasoning as `logos.LogoSource`.
...
- **mypy** — `strict`, over `src` *and* `tests`. Two overrides in `pyproject.toml`: Pillow needs `ignore_missing_imports` *locally*, and tests relax `disallow_untyped_defs`/`incomplete_defs`/`untyped_calls` because annotating every fake adds noise without catching bugs. Everything else stays strict.
  ...
  - **The pin was not what fixed the 3.13 abort.** One CI run died with a bare `Fatal Python error: Aborted` inside `PIL.Image.preinit` while `test_logos.py` saved a JPEG, and the identical commit passed on re-run — both on 12.3.0, so it was a race in Pillow's *lazy* plugin registration, not a bad release. `logos.LogoStore` runs daemon worker threads that call Pillow while the main thread does too, which is exactly the shape that triggers it. `logos.register_formats()` now runs `Image.init()` in `LogoStore.__init__` **before the threads are created**, so no worker can ever reach the lazy path.

    `Image.init()` and not `Image.preinit()`: preinit registers only BMP, DIB, GIF, JPEG, PNG and PPM. That covers every logo sampled from the live playlist (60 sampled: 53 JPEG, 7 PNG), but anything outside the six would fall back to the lazy path and quietly restore the race. `init()` costs ~25 ms once, registers all 44 formats, and is idempotent thereafter. It is called from `LogoStore.__init__` rather than at module import so a user with logos turned off pays nothing.
