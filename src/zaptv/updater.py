...
def download(path: Path = PLAYLIST_PATH, url: str = PLAYLIST_URL) -> Path:
    """Fetch the playlist, replacing the cache only once the body is in hand."""
...
def ensure(path: Path = PLAYLIST_PATH, max_age: int = MAX_AGE_SECONDS) -> Path:
    """Return a usable playlist path, downloading only when needed.
